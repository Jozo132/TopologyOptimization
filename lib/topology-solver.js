...
export class TopologySolver {
     ...
     *       'nonlinear' – nonlinear FEA with geometric/material nonlinearity (large deformation)
     *       'fracture'  – fracture/damage analysis (phase-field, CZM, element erosion)
     *       'dynamics'  – explicit dynamics for cutting/tearing simulations
     ...
     *   Nonlinear analysis parameters (when solver: 'nonlinear'):
     *   - materialType:   'linear'|'neohookean'|'j2plastic'|'druckerprager'|'mooneyrivlin'|'ogden'
     *   - loadSteps:      number of incremental load steps (default 10)
     *   - newtonTol:      Newton-Raphson convergence tolerance (default 1e-6)
     *   - maxNewtonIter:  max Newton iterations per step (default 20)
     *   Fracture analysis parameters (when solver: 'fracture'):
     *   - fractureMethod: 'phasefield'|'czm'|'erosion' (default 'phasefield')
     *   - damageModel:    'none'|'johnsoncook'|'gurson'|'lemaitre' (default 'none')
     *   - Gc:             critical energy release rate (J/m², default 2700)
     *   - lengthScale:    phase-field regularization length (default 2×element size)
     *   Dynamics parameters (when solver: 'dynamics'):
     *   - analysisType:   'explicit'|'implicit'|'newmark' (default 'explicit')
     *   - totalTime:      simulation duration (s, default 1.0)
     *   - density:        material density (kg/m³, default 7800 for steel)
     *   - safetyFactor:   CFL safety factor for explicit (default 0.8)
    ...
    optimize(model, config = {}, progressCallback) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (solverKey === '2d') {
                ...
                use3D = model.type === 'cube' || (model.nz != null && model.nz > 1);
            } else if (solverKey === 'nonlinear') {
                feaOnly = true;
                use3D = model.type === 'cube' || (model.nz != null && model.nz > 1);
            } else if (solverKey === 'fracture') {
                feaOnly = true;
                use3D = model.type === 'cube' || (model.nz != null && model.nz > 1);
            } else if (solverKey === 'dynamics') {
                feaOnly = true;
                use3D = model.type === 'cube' || (model.nz != null && model.nz > 1);
            } else {
            ...
            // feaSolverBackend (auto/webgpu/wasm/js) passes through via ...config spread
            if (feaOnly) workerConfig.feaOnly = true;
            if (solverKey === 'fatigue') workerConfig.fatigueMode = true;
            if (solverKey === 'nonlinear') workerConfig.nonlinearMode = true;
            if (solverKey === 'fracture') workerConfig.fractureMode = true;
            if (solverKey === 'dynamics') workerConfig.dynamicsMode = true;
    ...
    /**
     * Run nonlinear FEA with geometric/material nonlinearity.
     * Supports large deformation, hyperelastic, and elastoplastic materials.
     *
     * @param {object} model
     *   { nx, ny, nz, elements?: Float32Array|number[], type?: string }
     * @param {object} config
     *   Key parameters: materialType ('neohookean'|'j2plastic'|'druckerprager'|
     *   'mooneyrivlin'|'ogden'), loadSteps, newtonTol, maxNewtonIter,
     *   youngsModulus, poissonsRatio, yieldStress, hardeningModulus
     * @param {function} [progressCallback]
     * @returns {Promise<object>} Result with displacement, stress, and deformation data
     */
    analyzeNonlinear(model, config = {}, progressCallback) {
        return this.optimize(model, { ...config, solver: 'nonlinear' }, progressCallback);
    }

    /**
     * Run fracture/damage analysis.
     * Supports phase-field fracture, cohesive zone models, and element erosion.
     *
     * @param {object} model
     *   { nx, ny, nz, elements?: Float32Array|number[], type?: string }
     * @param {object} config
     *   Key parameters: fractureMethod ('phasefield'|'czm'|'erosion'),
     *   damageModel ('johnsoncook'|'gurson'|'lemaitre'|'none'),
     *   Gc (critical energy release rate), lengthScale (regularization length)
     * @param {function} [progressCallback]
     * @returns {Promise<object>} Result with damage field, crack path, and stress data
     */
    analyzeFracture(model, config = {}, progressCallback) {
        return this.optimize(model, { ...config, solver: 'fracture' }, progressCallback);
    }

    /**
     * Run explicit/implicit dynamics simulation.
     * Best for cutting/tearing with complex contact and severe nonlinearity.
     *
     * @param {object} model
     *   { nx, ny, nz, elements?: Float32Array|number[], type?: string }
     * @param {object} config
     *   Key parameters: analysisType ('explicit'|'implicit'|'newmark'),
     *   totalTime, density, safetyFactor, damping
     * @param {function} [progressCallback]
     * @returns {Promise<object>} Result with time history of displacement, stress, energy
     */
    analyzeDynamics(model, config = {}, progressCallback) {
        return this.optimize(model, { ...config, solver: 'dynamics' }, progressCallback);
    }
