#!/usr/bin/env node
/**
 * Benchmark: PETSc-style KSP Solver with BDDC, MG, and Jacobi Preconditioners
 *
 * Compares the performance of:
 *   1. JS KSP + Jacobi  — PCG with diagonal preconditioner (JavaScript)
 *   2. JS KSP + BDDC    — PCG with BDDC domain decomposition (JavaScript)
 *   3. JS KSP + MG      — PCG with Multigrid V-cycle preconditioner (JavaScript)
 *   4. WASM KSP + BDDC  — Full self-contained WASM BDDC solver
 *
 * Usage:
 *   node benchmark-ksp.js
 */
...
const E0 = 1;
const Emin = 1e-9;
const KSP_BDDC_SMOOTHER_ITERS = 3;
const MG_NU1 = 2;
const MG_NU2 = 2;
const MG_OMEGA = 0.5;
...
function setupProblem3D(nelx, nely, nelz) {
    ...
    // Apply unit force at right face center node (x=nelx)
...
// ═══════════════════════════════════════════════════════════════════════
// EbE matvec (shared by JS solvers)
// ═══════════════════════════════════════════════════════════════════════
function applyA_EbE(E_vals, activeElements, KEflat, edofArray, p, Ap, ndof) {
    Ap.fill(0);
...
// Build E_vals, active list, global inverse diagonal
function buildGlobalData(KEflat, edofArray, densities, penal, nel, ndof, fixedMask) {
    const dE = E0 - Emin;
    ...
    return { E_vals, active, invDiag };
}

// ═══════════════════════════════════════════════════════════════════════
// 1. JS KSP + Jacobi (diagonal preconditioner)
// ═══════════════════════════════════════════════════════════════════════
function solveKSP_Jacobi_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, ndof, F, fixedMask, tolerance, maxIter) {
    const nel = nelx * nely * nelz;
    const { E_vals, active, invDiag } = buildGlobalData(KEflat, edofArray, densities, penal, nel, ndof, fixedMask);
    ...
    for (let iter = 0; iter < maxIter; iter++) {
        ...
        if (rn2 < tolSq) break;
        iters++;
        applyA_EbE(E_vals, active, KEflat, edofArray, p, Ap, ndof);
    ...
    return { U, compliance: c, iters };
}

// ═══════════════════════════════════════════════════════════════════════
// 2. JS KSP + BDDC (domain decomposition preconditioner)
// ═══════════════════════════════════════════════════════════════════════
...
function applyBDDC_JS(subdomains, coarseMap, E_vals, activeElements, KEflat, edofArray, fixedMask, invDiag, r, z, ndof) {
    ...
    for (let s = 1; s < KSP_BDDC_SMOOTHER_ITERS; s++) {
        applyA_EbE(E_vals, activeElements, KEflat, edofArray, z, Ap, ndof);
        for (let i = 0; i < ndof; i++) {
...
// ═══════════════════════════════════════════════════════════════════════
// 3. JS KSP + MG (Multigrid V-cycle preconditioner)
// ═══════════════════════════════════════════════════════════════════════
...
function solveKSP_MG_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, ndof, F, freedofs, fixedMask, tolerance, maxIter) {
    const nel = nelx * nely * nelz;
    const dE = E0 - Emin;
    const skipT = Emin * 1000;
    const loc = new Float64Array(24);
...
// ═══════════════════════════════════════════════════════════════════════
// 4. WASM KSP + BDDC solver
// ═══════════════════════════════════════════════════════════════════════
function solveKSP_BDDC_WASM(KEflat, edofArray, densities, penal, nelx, nely, nelz, ndof, F, fixedMask, tolerance, maxIter) {
    const nel = nelx * nely * nelz;
    const { E_vals, active, invDiag } = buildGlobalData(KEflat, edofArray, densities, penal, nel, ndof, fixedMask);
    ...
    let totalBytes = 0;
    totalBytes += numSubs * BDDC_SUB_STRIDE;
    totalBytes += coarseMap.length * 4;
    totalBytes += 576 * 8;
    totalBytes += nel * 24 * 4;
    totalBytes += nel * 8;
    totalBytes += active.length * 4;
    totalBytes += ndof;
    totalBytes += ndof * 8;
    totalBytes += ndof * 8;
    totalBytes += ndof * 8;
    totalBytes += 7 * ndof * 8 + 192;
    for (const sub of subdomains) {
        totalBytes += sub.elements.length * 4;
        totalBytes += sub.dofs.length * 4;
        totalBytes += ndof * 8;
    }
    totalBytes += 8192;
    ...
    for (let si = 0; si < numSubs; si++) {
        ...
        dv.setInt32(descOff + 0, sub.elements.length, true);
        dv.setInt32(descOff + 4, sub.dofs.length, true);
        dv.setUint32(descOff + 8, elemOff, true);
        dv.setUint32(descOff + 12, dofsOff, true);
        dv.setUint32(descOff + 16, localInvDiagOff, true);
        dv.setInt32(descOff + 20, 0, true);
        dv.setInt32(descOff + 24, 0, true);
        dv.setInt32(descOff + 28, 0, true);
    }

    const iterations = wasmModule.exports.ebeKSP_BDDC(
        subsOff, numSubs, coarseOff, coarseMap.length,
        keOff, edofsOff, evalsOff, activeOff, active.length,
        fmOff, invDiagOff, fOff, uOff,
        ndof, maxIter, tolerance, KSP_BDDC_SMOOTHER_ITERS, workOff
    );
...
async function runBenchmark(nelx, nely, nelz, iterations = 3) {
    ...
    // ── Warm-up + correctness ──
    console.log('Warming up & verifying correctness...');
    const jacobiRef = solveKSP_Jacobi_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER);
    const bddcRef = solveKSP_BDDC_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER);
    const mgRef = solveKSP_MG_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.freedofs, prob.fixedMask, CG_TOL, CG_MAX_ITER);
    const wasmRef = solveKSP_BDDC_WASM(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER);

    // Verify all solvers produce finite solutions
    const allFinite = [jacobiRef, bddcRef, mgRef, wasmRef].every(r => Number.isFinite(r.compliance));
    const allNonZero = [jacobiRef, bddcRef, mgRef, wasmRef].every(r => Math.abs(r.compliance) > 1e-20);
    if (allFinite && allNonZero) {
        console.log('✓ All solvers produce finite, non-zero solutions');
    } else {
        console.log('✗ Warning: Some solvers failed correctness checks!');
    }
    console.log(`  CG iterations — Jacobi: ${jacobiRef.iters}  BDDC: ${bddcRef.iters}` +
        `  MG: ${mgRef.iters}  WASM BDDC: ${wasmRef.iterations}`);
    console.log(`  Compliance    — Jacobi: ${jacobiRef.compliance.toExponential(4)}` +
        `  BDDC: ${bddcRef.compliance.toExponential(4)}` +
        `  MG: ${mgRef.compliance.toExponential(4)}  WASM: ${wasmRef.compliance.toExponential(4)}`);

    // ── Benchmark each solver ──
    const solvers = [
        { name: 'JS Jacobi', fn: () => solveKSP_Jacobi_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER) },
        { name: 'JS BDDC', fn: () => solveKSP_BDDC_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER) },
        { name: 'JS MG', fn: () => solveKSP_MG_JS(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.freedofs, prob.fixedMask, CG_TOL, CG_MAX_ITER) },
        { name: 'WASM BDDC', fn: () => solveKSP_BDDC_WASM(KEflat, edofArray, densities, penal, nelx, nely, nelz, prob.ndof, prob.F, prob.fixedMask, CG_TOL, CG_MAX_ITER) },
    ];

    const timings = {};
    for (const solver of solvers) {
        const times = [];
        console.log(`Benchmarking ${solver.name}...`);
        ...
        timings[solver.name] = { avg, min, times };
        console.log(`  ${solver.name}: avg ${avg.toFixed(2)}ms  min ${min.toFixed(2)}ms`);
    }

    // ── Per-case summary ──
    const bestName = Object.keys(timings).reduce((a, b) => timings[a].avg < timings[b].avg ? a : b);
    console.log('─'.repeat(78));
    console.log(`🏆 Best: ${bestName} (${timings[bestName].avg.toFixed(2)}ms avg)`);

    return {
        nelx, nely, nelz,
        nel: prob.nel,
        ndof: prob.ndof,
        nfree: prob.freedofs.length,
        jacobiAvg: timings['JS Jacobi'].avg,
        bddcAvg: timings['JS BDDC'].avg,
        mgAvg: timings['JS MG'].avg,
        wasmBddcAvg: timings['WASM BDDC'].avg,
        bestName,
        jacobiIters: jacobiRef.iters,
        bddcIters: bddcRef.iters,
        mgIters: mgRef.iters,
        wasmIters: wasmRef.iterations,
    };
}
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║  KSP Solver Benchmark: Jacobi vs BDDC vs MG vs WASM BDDC                   ║');
    console.log('║  PETSc-style preconditioned conjugate gradient comparison                   ║');
    console.log('╚══════════════════════════════════════════════════════════════════════════════╝');
    ...
    // ── Summary table ──
    console.log('\n\n' + '═'.repeat(110));
    console.log('SUMMARY: KSP Solver Comparison (all times in ms, averaged)');
    console.log('═'.repeat(110));
    console.log('Mesh         | Elements | Free DOFs | Jacobi   | BDDC     | MG       | WASM BDDC | Best');
    console.log('─'.repeat(110));
    ...
    for (const r of results) {
        ...
        const jacStr = r.jacobiAvg.toFixed(2).padStart(8);
        const bddcStr = r.bddcAvg.toFixed(2).padStart(8);
        const mgStr = r.mgAvg.toFixed(2).padStart(8);
        const wasmStr = r.wasmBddcAvg.toFixed(2).padStart(9);
        console.log(`${meshStr} | ${elStr} | ${freeStr} | ${jacStr}` +
            ` | ${bddcStr} | ${mgStr} | ${wasmStr} | ${r.bestName}`);
    }
    ...
    // CG iteration counts
    console.log('\nCG Iteration Counts:');
    console.log('Mesh         | Jacobi  | BDDC    | MG      | WASM BDDC');
    console.log('─'.repeat(60));
    for (const r of results) {
        const meshStr = `${r.nelx}×${r.nely}×${r.nelz}`.padEnd(12);
        console.log(`${meshStr} | ${String(r.jacobiIters).padStart(7)} | ${String(r.bddcIters).padStart(7)} | ${String(r.mgIters).padStart(7)} | ${String(r.wasmIters).padStart(9)}`);
    }

    // Speedup relative to JS Jacobi (the baseline)
    console.log('\nSpeedup vs JS Jacobi:');
    for (const r of results) {
        const meshStr = `${r.nelx}×${r.nely}×${r.nelz}`;
        const bddcSpeedup = (r.jacobiAvg / r.bddcAvg).toFixed(2);
        const mgSpeedup = (r.jacobiAvg / r.mgAvg).toFixed(2);
        const wasmSpeedup = (r.jacobiAvg / r.wasmBddcAvg).toFixed(2);
        console.log(`  ${meshStr}: BDDC ${bddcSpeedup}x  MG ${mgSpeedup}x  WASM BDDC ${wasmSpeedup}x`);
    }
